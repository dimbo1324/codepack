...
from ..models import RiskPreviewItem
...
class ExportPlan:
    ...
    def large_files(self) -> list[PlannedFile]:
        return [item for item in self.included_files + self.excluded_files if item.size >= 100 * 1024 * 1024]
...
def _classify_group(path: Path) -> str:
    ...
    if suffix in {'md', 'rst', 'adoc', 'txt'} or name in {'readme.md', 'license'}:
        return 'docs'
    if suffix in {'json', 'yaml', 'yml', 'toml', 'ini', 'cfg', 'conf', 'lock'} or name.startswith('dockerfile'):
...
def _selected_by_diff_and_incremental(rel_key: str, diff_selection: DiffSelection, incremental: IncrementalSelection) -> bool:
...
def build_export_plan(
    ...
    for current_dir, dirnames, filenames in os.walk(source_root, topdown=True, followlinks=False):
        ...
        for filename in filenames:
            ...
            if not _selected_by_diff_and_incremental(rel_key, diff_selection, incremental_selection):
                plan.excluded_files.append(PlannedFile(rel_key, size, 'excluded', 'not selected by diff/incremental mode', 'info', group))
                continue
            skip_by_rule, reason = export_rules.should_skip_file(relative_path)
            if skip_by_rule:
                plan.excluded_files.append(PlannedFile(rel_key, size, 'excluded', reason, 'medium', group))
                continue
            safety = should_skip_file_for_safety(relative_path, config.normalized_safe_export_mode())
            if safety.skip:
                plan.excluded_files.append(PlannedFile(rel_key, size, 'excluded', safety.reason, safety.severity, group))
                continue
...
def write_export_plan_files(plan: ExportPlan, output_json: Path, output_md: Path) -> None:
    ...
    output_json.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding='utf-8', newline='\n')
    ...
    with output_md.open('w', encoding='utf-8', newline='\n') as out:
        ...
        out.write(f'- Estimated selected project size: {format_bytes(plan.estimated_included_bytes)}\n\n')
        if plan.warnings:
            out.write('## Warnings\n\n')
        ...
        if not sensitive:
            ...
            for item in sensitive:
                out.write(f'- [{item.severity}] `{item.relative_path}` — {item.reason} ({format_bytes(item.size)})\n')
        out.write('\n## Large files\n\n')
        ...
        if not large:
            ...
            for item in large:
                out.write(f'- `{item.relative_path}` — {format_bytes(item.size)} — {item.status} — {item.reason or item.group}\n')
...
def format_export_plan_for_user(plan: ExportPlan, zip_limit_bytes: int) -> str:
    ...
    if sensitive:
        lines.extend(f'- [{item.severity}] {item.relative_path}: {item.reason}' for item in sensitive)
    else:
        lines.append('- none')
    ...
    if large:
        lines.extend(f'- {item.relative_path}: {format_bytes(item.size)} ({item.status})' for item in large)
    else:
