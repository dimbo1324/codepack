...
_RU: dict[str, str] = {
    ...
    # Standard dialog buttons (Qt defaults are English-only, so we set them explicitly)
    "btn.yes": "Да",
    "btn.no": "Нет",
    "btn.ok": "ОК",
    # QFileDialog filters
    "dialog.json_filter": "JSON-файлы (*.json)",
    # Path validation errors (raised in utils.path_utils, shown via message boxes)
    "error.path.empty": "Укажите корневую папку проекта.",
    "error.path.not_exist": "Указанная папка не существует.",
    "error.path.not_dir": "Указанный путь не является папкой.",
    "error.path.drive_root": "Нельзя выбирать корень диска.",
    "error.path.home": "Не выбирайте всю домашнюю папку целиком. Укажите конкретный проект.",
    # Log-page messages (visible on the Log page, must follow the active language)
    "log.ready": "{name} v{version} готов к работе.",
    "log.choose_folder": "Выберите папку проекта и создайте экспорт-пакет.",
    "log.building_plan": "Строится план экспорта... профиль={profile}, режим={mode}, diff={diff}",
    "log.export_thread_start": "Запуск потока экспорта...",
    "log.cancel_requested": "Запрошена отмена...",
    "log.diagnostic_saved": "Технические подробности записаны в: {log}",
    "log.rules_saved": "Правила включения/исключения сохранены.",
    "log.goals_saved": "Цели промптов сохранены.",
    "log.exportignore_created": "Создан .exportignore: {path}",
    "log.settings_exported": "Настройки экспортированы: {path}",
    "log.settings_imported": "Настройки импортированы: {path}",
    "log.settings_reset": "Настройки сброшены к значениям по умолчанию.",
    "analytics.error_generic": "Не удалось собрать аналитику. Подробности записаны в журнал.",
    # HistoryDialog entry fields
    "history.dlg_project": "Проект: {name}",
    "history.dlg_meta": "Профиль: {profile}; Режим: {mode}; Разбивка: {split}",
    "history.dlg_result": "Результат: {result}",
    # Clipboard dump headers (copied to clipboard for the AI assistant)
    "clipboard.context_header": "ЗАДАЧА / КОНТЕКСТ РАЗРАБОТЧИКА",
    "clipboard.truncated": "[ОБРЕЗАНО: достигнут лимит 20 МБ для буфера обмена]",
    "clipboard.file_label": "ФАЙЛ",
    # Tray / watcher
...
_EN: dict[str, str] = {
    ...
    # Path validation errors (raised in utils.path_utils, shown via message boxes)
    "error.path.empty": "Specify the project root folder.",
    "error.path.not_exist": "The specified folder does not exist.",
    "error.path.not_dir": "The specified path is not a folder.",
    "error.path.drive_root": "You cannot select a drive root.",
    "error.path.home": "Do not select your entire home folder. Choose a specific project.",
    # Log-page messages (visible on the Log page, must follow the active language)
    "log.ready": "{name} v{version} is ready.",
    "log.choose_folder": "Select a project folder and create an export package.",
    "log.building_plan": "Building export plan... profile={profile}, mode={mode}, diff={diff}",
    "log.export_thread_start": "Starting export thread...",
    "log.cancel_requested": "Cancellation requested...",
    "log.diagnostic_saved": "Technical details saved to: {log}",
    "log.rules_saved": "Include/exclude rules saved.",
    "log.goals_saved": "Prompt goals saved.",
    "log.exportignore_created": "Created .exportignore: {path}",
    "log.settings_exported": "Settings exported: {path}",
    "log.settings_imported": "Settings imported: {path}",
    "log.settings_reset": "Settings reset to defaults.",
    "analytics.error_generic": "Could not collect analytics. Details saved to the log.",
    # HistoryDialog entry fields
    "history.dlg_project": "Project: {name}",
    "history.dlg_meta": "Profile: {profile}; Mode: {mode}; Split: {split}",
    "history.dlg_result": "Result: {result}",
    # Clipboard dump headers (copied to clipboard for the AI assistant)
    "clipboard.context_header": "TASK / DEVELOPER CONTEXT",
    "clipboard.truncated": "[TRUNCATED: 20 MB clipboard limit reached]",
    "clipboard.file_label": "FILE",
