...
_FASTAPI_RE = re.compile(
    r"@(?:app|router)\.(get|post|put|patch|delete|options|head)\(\s*['\"]([^'\"]+)", re.IGNORECASE
)
_FLASK_RE = re.compile(
    r"@(?:app|blueprint|bp)\.route\(\s*['\"]([^'\"]+).*?(?:methods\s*=\s*\[([^\]]+)\])?",
    re.IGNORECASE,
)
_EXPRESS_RE = re.compile(
    r"(?:app|router)\.(get|post|put|patch|delete|use)\(\s*['\"]([^'\"]+)", re.IGNORECASE
)
...
def write_api_surface_report(
    ...
    with output_file.open("w", encoding="utf-8", newline="\n", errors="replace") as out:
        ...
        if backend_routes:
            for path, method, route in sorted(
                set(backend_routes), key=lambda item: (item[2], str(item[0]).lower())
            )[:500]:
        ...
        if frontend_calls:
            for path, call in sorted(
                set(frontend_calls), key=lambda item: (item[1], str(item[0]).lower())
            )[:500]:
