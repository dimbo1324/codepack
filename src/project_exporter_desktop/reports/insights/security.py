...
def _collect_security_findings(copied_root: Path, max_bytes_per_file: int | None) -> tuple[list[tuple[str, Path]], list[SecretFinding], list[RiskyCodeFinding]]:
...
def _write_security_json(
    ...
    output_file.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8", newline="\n")
...
def _write_security_sarif(json_file: Path, sarif_file: Path) -> None:
    ...
    sarif = {
        ...
        "runs": [{
            "tool": {"driver": {"name": "Project Exporter Security Scanner", "rules": list(rules.values())}},
            "results": results,
        }],
    }
    sarif_file.write_text(json.dumps(sarif, ensure_ascii=False, indent=2), encoding="utf-8", newline="\n")


def write_security_scan_report(copied_root: Path, output_file: Path, max_bytes_per_file: int | None) -> None:
    suspicious_files, suspicious_lines, risky_code = _collect_security_findings(copied_root, max_bytes_per_file)
    ...
    with output_file.open("w", encoding="utf-8", newline="\n", errors="replace") as out:
        ...
        out.write("Machine-readable outputs are written next to this report: 06_security_scan.json and 06_security_scan.sarif.\n")
