...
def write_html_dashboard(reports_dir: Path, output_file: Path) -> None:
    ...
    cards = [
        ...
        ('Security findings', str(security_summary.get('total_findings', 'n/a')), 'JSON + SARIF are generated'),
        ('Potential secrets', str(security_summary.get('potential_secrets', 'n/a')), 'Redacted secret-like lines'),
        ('Archive strategy', 'split' if 'Split planned: `True`' in _read_text(archive_plan, 2000) else 'single', 'Logical ZIP planning'),
    ]
    links = [
