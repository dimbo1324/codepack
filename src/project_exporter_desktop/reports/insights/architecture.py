...
_LAYER_HINTS: dict[str, tuple[str, ...]] = {
    ...
    "Application / orchestration": ("services", "application", "usecases", "use_cases", "handlers", "controllers"),
    "Domain / business logic": ("domain", "core", "models", "entities", "business"),
    "Data access / persistence": ("repositories", "repository", "dao", "db", "database", "migrations", "storage"),
...
def _find_entrypoints(root: Path) -> list[Path]:
    ...
    for path in iter_project_files(root):
        ...
        if name in _ENTRYPOINT_NAMES or name.startswith("vite.config") or name.startswith("next.config"):
            result.append(path)
...
def write_architecture_report(copied_root: Path, output_file: Path, inventory: dict[str, Any]) -> None:
    ...
    with output_file.open("w", encoding="utf-8", newline="\n", errors="replace") as out:
        ...
        out.write("This report is a static architecture map based on file/folder conventions and detected tooling.\n\n")
        ...
        out.write("\n## Extension points\n\n")
        out.write("- Add new report modules under `reports/insights/` and register them in `orchestrator.py`.\n")
        out.write("- Add new copy/exclusion rules in `config.py`, `constants.py`, and `copy_service.py`.\n")
        out.write("- Add UI options in `ui/app_window.py`; keep long-running work in the exporter thread.\n")
        out.write("- Keep generated-report writers pure: input paths in, report file out.\n")

        out.write("\n## Potential architectural risks\n\n")
        if not layer_dirs:
            out.write("- No conventional layers detected; project may be very small or unconventionally organised.\n")
        if any(p.name == "app_window.py" for p in inventory["files"]):
            out.write("- Desktop UI code may grow quickly; consider splitting layout, state sync, and worker orchestration when it becomes large.\n")
        out.write("- Validate this report manually before making structural refactors; it is heuristic.\n")
