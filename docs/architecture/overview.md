...
| Гейт качества (`cargo xtask gate`) | **зелёный локально и в CI**: fmt, clippy `-D warnings`, тесты, `cargo deny check`, фронтенд `format`/`typecheck`/`lint`, `sync-agents --check`. Фронтенд-шаги пропускаются с уведомлением без `node_modules`, но при выставленной переменной `CI` **падают** — иначе удаление шага `pnpm install` из воркфлоу прошло бы незаметно |
