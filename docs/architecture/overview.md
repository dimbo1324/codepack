...
- Cancelling while a single very large file is being packed into an archive is not
  interruptible until that file finishes. Characterized, not fixed, by
  `crates/codepack-archive/tests/cancellation_mid_file.rs` (`#[ignore]`-gated, run in
  `perf-smoke-weekly.yml`) — audit 2026-09-07, `04-TESTS.txt` item 12.
- The `codepack-desktop` binary in the `.deb`/`.rpm` is still built natively on
  `ubuntu-latest`, unlike `codepack-cli` (see the Packaging row): it carries the same
  `GLIBC_2.39` requirement and so almost certainly fails to start on Debian 12 the same
  way the CLI did before 2026-09-08's fix. Not cross-built the same way in this pass:
  `install-and-run-linux`'s GUI-under-Xvfb check is `continue-on-error` (soft, per its
  own comment), so nothing gates on it today, and cross-building the whole
  webview/GTK/appindicator stack for an older base image is a meaningfully bigger,
  separately-risked change than the CLI's plain `cargo build`. Left open rather than
  guessed at.
