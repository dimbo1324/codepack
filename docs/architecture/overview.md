...
Этап **S10 сдан**: `codepack-cli` — рабочий headless-бинарь, первая точка входа транша
«Интерфейс». Плейсхолдеров в воркспейсе не осталось.
...
Этап **S11 — Tauri-оболочка и UI** (`ROADMAP.md` §3). Появляется
`apps/desktop/src-tauri`, которого сейчас нет; `apps/desktop/ui` (TypeScript) уже
существует как каркас. Инвариант I8 (ядро не зависит от интерфейса) станет проверяемым
на практике впервые: до сих пор его никто не мог нарушить, потому что интерфейса не
было.
