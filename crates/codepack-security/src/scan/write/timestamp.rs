...
//! `codepack-scanner::plan::timestamp` for the same out-of-scope-dependency reason as
//! `crate::classify` (Q7, `docs/decisions/open-questions.md`): this crate cannot
//! depend on `codepack-scanner`. Renders a UTC timestamp rather than legacy's local
//! wall clock — a deliberate, documented deviation for a cosmetic, non-contractual
//! field (nothing parses `generated_at` back).
