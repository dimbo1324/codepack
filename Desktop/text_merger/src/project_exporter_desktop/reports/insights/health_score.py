...
def write_project_health_report(copied_root: Path, output_file: Path, inventory: dict[str, Any]) -> None:
    ...
    suspicious_names = [p for p in files if p.name.casefold().startswith('.env') or 'secret' in p.name.casefold() or 'credential' in p.name.casefold()]
    lockfiles = [p for p in files if p.name in {'pnpm-lock.yaml', 'package-lock.json', 'yarn.lock', 'poetry.lock', 'uv.lock', 'go.sum', 'Cargo.lock'}]
    ...
    if any('src' in p.parts for p in files):
        architecture += 12; reasons['Architecture'].append('src/ style source layout detected')
    if any(part in {'services', 'domain', 'core', 'utils', 'reports'} for p in files for part in p.parts):
        architecture += 10; reasons['Architecture'].append('separated service/core utility modules detected')
    if len(source_files) > 0 and len(source_files) < max(1, len(files)):
        architecture += 6; reasons['Architecture'].append('source files are not mixed with every exported file')
    if len([p for p, _ in large_files if extension_key(p) in SOURCE_CODE_EXTENSIONS]) > 3:
        architecture -= 8; reasons['Architecture'].append('several large source files need decomposition')

    security = 82
    reasons['Security'] = []
    if suspicious_names:
        penalty = min(45, 12 * len(suspicious_names))
        security -= penalty; reasons['Security'].append(f'{len(suspicious_names)} sensitive-looking filenames detected')
    else:
        security += 5; reasons['Security'].append('no obvious sensitive filenames in exported copy')
    if lockfiles:
        security += 3; reasons['Security'].append('dependency lockfile(s) detected')

    maintainability = 64
    reasons['Maintainability'] = []
    if docs:
        maintainability += 10; reasons['Maintainability'].append('documentation files are present')
    if configs:
        maintainability += 8; reasons['Maintainability'].append('standard config files are present')
    ...
    if stack.get('testing'):
        tests += 18; reasons['Testing'].append('testing framework detected')
    if test_files:
    ...
    if (copied_root / 'README.md').exists():
        documentation += 15; reasons['Documentation'].append('README.md exists')
    if (copied_root / 'docs').exists():
        documentation += 7; reasons['Documentation'].append('docs/ folder exists')
    if not docs:
    ...
    if lockfiles:
        dep_hygiene += 18; reasons['Dependency Hygiene'].append('lockfiles improve reproducibility')
    if stack.get('package_managers'):
        dep_hygiene += 8; reasons['Dependency Hygiene'].append('package manager detected')
    if len(stack.get('package_managers', [])) > 2:
        dep_hygiene -= 8; reasons['Dependency Hygiene'].append('multiple package managers may increase maintenance cost')

    ai_readiness = 78
    reasons['AI Readiness'] = []
    if (copied_root.parent / 'PROJECT_PROFILE.json').exists():
        ai_readiness += 8; reasons['AI Readiness'].append('PROJECT_PROFILE.json is available')
    if source_files:
        ai_readiness += 5; reasons['AI Readiness'].append('source files are included')
    if suspicious_names:
        ai_readiness -= 12; reasons['AI Readiness'].append('sensitive-looking files reduce safe sharing readiness')

    export_safety = 92
    reasons['Export Safety'] = []
    if suspicious_names:
        export_safety -= min(30, 10 * len(suspicious_names)); reasons['Export Safety'].append('review sensitive-looking files before sharing')
    else:
