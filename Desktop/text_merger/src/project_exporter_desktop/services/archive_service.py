...
def _iter_archive_entries(paths: ExportPaths, include_project: bool) -> tuple[list[ArchiveEntry], int]:
    ...
    for file_path in paths.staging_dir.rglob('*'):
        ...
        try:
            ...
            size = 0
        entries.append(ArchiveEntry(file_path, arcname, size, classify_archive_group(arcname, paths.project_name)))
...
def _plan_logical_parts(entries: list[ArchiveEntry], target_bytes: int) -> list[ArchivePartPlan]:
    ...
    def flush() -> None:
        ...
        parts.append(ArchivePartPlan(index=len(parts) + 1, group_hint=group_hint, entries=current_entries))
        current_entries = []
        current_size = 0
        current_group = ''
    ...
    for group in sorted(grouped):
        for entry in grouped[group]:
            ...
            if current_entries and (current_group != group or current_size + entry.size > target_bytes):
                flush()
...
def build_archive_plan(paths: ExportPaths, include_project: bool, part_limit_bytes: int = MAX_ARCHIVE_PART_BYTES) -> ArchivePlan:
    ...
    return ArchivePlan(True, limit_bytes, target_bytes, skipped_project_files, entries, _plan_logical_parts(entries, target_bytes))
...
def _write_restore_files(paths: ExportPaths, result: ArchiveBuildResult, part_map: list[dict[str, object]], limit_bytes: int) -> None:
...
def write_archive_plan_report(plan: ArchivePlan, output_file: Path) -> None:
    ...
    output_file.with_suffix('.json').write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding='utf-8', newline='\n')
...
def build_final_archives(
    ...
    if not plan.split:
        ...
        if cancel.is_set():
            return ArchiveBuildResult([paths.final_zip] if paths.final_zip.exists() else [], None, False, count, plan.skipped_project_files)
        ...
        if compressed_size <= plan.limit_bytes:
            ...
            return ArchiveBuildResult([paths.final_zip], None, False, count, plan.skipped_project_files)
        # Very rare, but possible due to ZIP overhead/incompressible data.
        log(f'Одиночный ZIP превысил лимит после записи ({format_bytes(compressed_size)} > {format_bytes(plan.limit_bytes)}). Пересобираю частями.')
        try:
    ...
    for part in plan.parts:
        ...
        if compressed_size > plan.limit_bytes:
            oversized = [str(entry.arcname).replace('\\', '/') for entry in part.entries if entry.size >= plan.target_bytes]
        ...
        log(f'Создан архив-часть {part.index}: {archive_name} ({count:,} файлов, {format_bytes(compressed_size)})')
